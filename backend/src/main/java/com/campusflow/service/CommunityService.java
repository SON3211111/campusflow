...
public class CommunityService {
    ...
    public List<CommunityPostResponse> getAllPosts() {
        return communityPostRepository.findAllByOrderByCreatedAtDesc().stream()
                .map(p -> new CommunityPostResponse(
                        ...
                        p.getAuthor().getName(), p.getWorkspace().getName(),
                        p.isSolved(), p.getCreatedAt(), null))
                .toList();
    }
    ...
    public CommunityPostResponse getPostDetail(String postId) {
        ...
        return new CommunityPostResponse(
                ...
                post.getAuthor().getName(), post.getWorkspace().getName(),
                post.isSolved(), post.getCreatedAt(), comments
        );
    }

    // 4. 도움 요청 글 삭제 (CASCADE 조건에 의해 DB 내 연관 댓글/대댓글 일괄 연쇄 삭제)
